...
class Reports extends CI_Controller {
	...
	public function masterlist()
	{
		if ($this->session->userdata['userdata']['permissionLevel'] == 2) {
			...
			$applications = $this->Issued_Application_m->get_unique_issued_applications();

			foreach ($applications as $key => $app) {
				$data['applications'][] = new BPLO_Application($app->referenceNum);
			}

			$this->load->view('dashboard/bplo/masterlist', $data);
		}
	}
